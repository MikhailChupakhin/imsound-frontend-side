  ...
  methods: {
    ...
    async handleComparison(productInfo, event) {
      ...
      if (productIndex !== -1) {
        ...
        const message = 'Товар удален из списка для сравнения';
        console.log(message);
        showError(message, x, y);
      } else {
        comparisonList.commit('addItem', { productInfo });
        const message = 'Товар добавлен в список для сравнения';
        console.log(message);
        showError(message, x, y);
      }
    }
...
.product-card:hover .vertical-card-menu {
  opacity: 1;
}
.card-icon:hover {
  fill: #8B0000;
}
.vertical-card-menu .icon {
  width: 1.5rem;
  height: 1.5rem;
  fill: #000;
  margin-bottom: 10px;
